...
do
    ...
    do
        grep -e "$type.*+$" output/intergenic/intergenic.split.bed > plus.bed
        grep -e "$type.*-$" output/intergenic/intergenic.split.bed > minus.bed
        ...
        do
            ...
            samtools view -b -h $plus_samtools_arg 16 -L plus.bed $BAMFILE > $prefix.1.bam
            samtools view -b -h $minus_samtools_arg 16 -L minus.bed $BAMFILE > $prefix.2.bam
